function ProcessGrid(obj, grid, Xmax, Ymax, R, C)
...
%   nursery: Nursery Object, use AddTree Function
   ...
   % imshow(grid)
    ...
    for k=2:n
        ...
        [~,by_max] = IJtoXY(max(b(:,2)),max(b(:,1)),Xmax,Ymax,R,C);
        [~,by_min] = IJtoXY(min(b(:,2)),min(b(:,1)),Xmax,Ymax,R,C);
    ...
    values = 1:obj.nRows;
    tree_list = [tree_xs;tree_ys;tree_ds;discretize(tree_xs,obj.nRows,values)]';
%     figure
%     plot(tree_list(:,1),tree_list(:,2),'*');
    for k = 1:length(tree_list)
       obj.AddTree(tree_list(k,1),tree_list(k,2),tree_list(k,3),tree_list(k,4));
    end
end
